...
;WITH GenreMoviesPerDirectory AS (
    SELECT P2.person_id, P2.first_name, P2.last_name, PG.genre_name, COUNT(PG.genre_name) AS TotalGenre, SUM(   )
    FROM Movie_Director
             INNER JOIN Product P on Movie_Director.product_id = P.product_id
             INNER JOIN Person P2 on Movie_Director.person_id = P2.person_id
             INNER JOIN Product_Genre PG on P.product_id = PG.product_id
    GROUP BY P2.person_id, P2.first_name, P2.last_name, PG.genre_name
)

SELECT person_id, first_name, last_name , genre_name, MAX(Total), SUM(Total)
FROM GenreMoviesPerDirectory
GROUP BY person_id, first_name, last_name, genre_name
